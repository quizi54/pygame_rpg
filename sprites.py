...
class Ground(pygame.sprite.Sprite):
    def __init__(self, game, x, y):
        ...
        x_choice = random.randint(0, 100)
        if x_choice <= 10:
            x_choice = random.randint(3, 5) * 32
        elif 11<= x_choice <= 60:
            x_choice = random.randint(0, 1) * 32
        else:
            x_choice = 2 * 32
        self.image = self.game.terrain_spritesheet.get_sprite(x_choice, 352, self.width, self.height)
        self.rect = self.image.get_rect()
        self.rect.x = self.x
        self.rect.y = self.y
